...
)
{
    ...
    if (pHashEntry != NULL)
    {
        ...
        if (pTempProcess->ProcessHandle == ProcessHandle)
        {
            HashTableRemove(&pCurrentProcess->ChildProcessTable, (PHASH_KEY)&ProcessHandle);
            ProcessCloseHandle(pTempProcess);
            return STATUS_SUCCESS;
        }
    }
    return STATUS_UNSUCCESSFUL;
...
)
{
    ...
    STATUS createFileStatus = IoCreateFile(&pFileObject, absolutePath, Directory, Create, FALSE);
    if (SUCCEEDED(createFileStatus))
    {
        ++pCurrentProcess->CurrentMaximumHandle;
        pFileObject->FileHandle = pCurrentProcess->CurrentMaximumHandle;
        *FileHandle = pCurrentProcess->CurrentMaximumHandle;
        HashTableInsert(&pCurrentProcess->FileTable, &pFileObject->FileTableEntry);
    }
...
)
{
    if (FileHandle == UM_FILE_HANDLE_STDOUT && GetCurrentProcess()->IsStdoutOpen)
    {
        GetCurrentProcess()->IsStdoutOpen = FALSE;
        return STATUS_SUCCESS;
    }  
    else if (FileHandle == UM_FILE_HANDLE_STDOUT && !GetCurrentProcess()->IsStdoutOpen)
    {
        return STATUS_INVALID_PARAMETER1;
    }
    else if (FileHandle == UM_INVALID_HANDLE_VALUE ||
        FileHandle > 64 ||
        FileHandle < 0)
    {
        return STATUS_INVALID_PARAMETER1;
    ...
    else
    {
        ...
        PHASH_ENTRY pHashEntry = HashTableLookup(&pCurrentProcess->FileTable, (PHASH_KEY)&FileHandle);
        if (pHashEntry != NULL)
        {
            PFILE_OBJECT pTempFile = CONTAINING_RECORD(pHashEntry, FILE_OBJECT, FileTableEntry);
            if (pTempFile->FileHandle == FileHandle)
            {
                HashTableRemove(&pCurrentProcess->FileTable, (PHASH_KEY)&FileHandle);
                STATUS StatusCloseFile = IoCloseFile(pTempFile);
                if (!SUCCEEDED(StatusCloseFile))
                {
                    return STATUS_UNSUCCESSFUL;
                }
                return StatusCloseFile;
            }
        }
        return STATUS_UNSUCCESSFUL;
    }
...
)
{
    ...
    if (pHashEntry != NULL)
    {
        ...
        if (pTempFile->FileHandle == FileHandle)
        {
            STATUS StatusReadFile = IoReadFile(pTempFile, BytesToRead, &pTempFile->CurrentByteOffset, Buffer, BytesRead);
            if (!SUCCEEDED(StatusReadFile))
            {
                return STATUS_UNSUCCESSFUL;
            }
            return StatusReadFile;
        }
    }
    return STATUS_UNSUCCESSFUL;
}
